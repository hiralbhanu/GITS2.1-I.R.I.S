...
def _update(cmd, path):
    if os.path.exists(path):
        ...
        create = input("{} Command does not exists. ".format(cmd) +
            "Do you want to create {} command?(y/n)".format(cmd))
...
def gits_custom(args):
    try:
        ...
        gits_logging.gits_logger.error("gits custom command caught an "
            + "exception")
